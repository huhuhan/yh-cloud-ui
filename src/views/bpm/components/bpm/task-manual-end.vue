...
<script>
  ...
  export default {
    ...
    watch: {
      task(newVal, oldVal) {
        this.$log.primary(`流程【${this.title}】窗口-任务对象发生变化，new-id： ${newVal ? newVal.id : null}`)
      }
    },
    methods: {
      close() {
      ...
      openInit() {
        this.form.opinion = ''
        this.$log.default(`初始化【${this.title}】窗口`)
      },
      handleConfirm() {
        BpmTaskDoAction({
          action: BpmTaskAction.manualEnd.key,
